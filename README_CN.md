...
Mac 终端会打印 Mac 控制台 URL，以及一个或多个 Windows 配对 URL，例如：
...
控制台可以看到已连接的 LAN Agent、截图、双方消息、Jobs、上传日志和命令输出。
你也可以在这里给 Windows 页面发消息、执行 PowerShell、请求截图，以及启动任务
模板。

在 Windows 电脑上：

1. 用 Windows 浏览器打开 Mac 终端打印出来的 URL。
2. 页面第一块就是 **Copy and run this Agent command**。
3. 点击 **Copy Agent Command**。
4. 把复制出来的命令粘贴到 Windows PowerShell 并运行。
5. 保持这个 PowerShell 窗口打开，Mac 才能继续控制 Windows。
6. 如果需要从 Windows 告诉 Mac 当前发生了什么，可以使用 **Send a message to Mac**。
...
当 Windows Agent 显示已连接后，回到 Mac 控制台，或使用 CLI：
...
```text
Mac 启动 server -> Mac 打开 /console -> Windows 浏览器打开配对 URL -> Windows 复制并运行 Agent 命令 -> Mac 控制 Windows
```
...
- 消息、Jobs、Agent、设备和日志：`.state/state.json`
